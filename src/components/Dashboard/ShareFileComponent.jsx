...
export default function ShareFileComponent({ onFileUploaded }) {
  ...
  const [selectedFiles, setSelectedFiles] = useState(() => Array.isArray(initialFiles) ? initialFiles : []);
    ...
    if (location.state) {
      ...
      if (navFiles.length > 0) {
        setSelectedFiles((prev) => {
          const merged = [...prev];
          navFiles.forEach((nf) => {
            const exists = merged.some((ef) => ef.name === nf.name && ef.size === nf.size);
  ...
  const handleShareClick = async () => {
    ...
    const url = `https://cloudnest.com/share/${Date.now()}`;
    setShareUrl(url);
    ...
    selectedFiles.forEach((file) => {
      ...
      try {
        if (typeof onFileUploaded === "function") onFileUploaded(fileMeta);
        else {
          const prev = JSON.parse(localStorage.getItem("recentFiles") || "[]");
          localStorage.setItem("recentFiles", JSON.stringify([fileMeta, ...prev].slice(0, 10)));
        }
      } catch (err) {
        console.error("onFileUploaded threw", err);
      }
    });
  ...
  const formatFileSize = (bytes) => {
    ...
    return `${(bytes / Math.pow(k, val)).toFixed(2)} ${sizes[val] || "Bytes"}`;
  };
        ...
        {uploadStatus === "success" && (
          <div className="mb-6 card p-6 border-green-200 bg-green-50">
            <div className="flex items-center gap-3 mb-4">
              ...
              <span className="font-medium text-green-800">Upload successful! Files added to recent files.</span>
            </div>
            <div className="flex items-center space-x-2">
              <div className="bg-gray-100 rounded-lg p-3 font-mono text-sm break-all flex-1">{shareUrl}</div>
              <button
          ...
          <div className="text-center">
            <button
              ...
              className={`w-24 h-24 flex items-center justify-center transition-all duration-300 shadow-lg hover:shadow-xl ${
                !hasFiles || uploadStatus === "uploading" ? "bg-gray-300 cursor-not-allowed" : "bg-blue-500 hover:bg-blue-600"
              }`}
            >
          ...
          <div className="card p-6 mb-8">
            ...
            <div className="space-y-3 max-h-64 overflow-y-auto">
              {selectedFiles.map((file, index) => (
                <div key={`${file.name}-${file.size}-${index}`} className="flex items-center justify-between p-3 bg-gray-50 rounded-xl">
